...
- **Tolerant response parser.** `lib/quicknode/sql.ts` accepts `{rows}`, `{data}`, `{result}`, or a bare array. Narrow this once the contract is fully confirmed.
...
- **Staking events still come from SQL polling.** The proto's `EVENTS` stream is described as "funding, liquidations" — not the staking program (`CDeposit` / `CWithdrawal` / `Delegation`). SQL `stakingEvents` query at 60s cadence remains the correct source; latency 0–60s. Acceptable for institutional audience.
